...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:maykinmedia/pta_export.git
       $ cd pta_export
...
``src/pta_export/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test pta_export
...
* ``DB_NAME``: name of the database for the project. Defaults to ``pta_export``.
* ``DB_USER``: username to connect to the database with. Defaults to ``pta_export``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``pta_export``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting pta_export_db_1 ... done
       Starting pta_export_web_1 ... done
   ...
   .. code-block:: bash
       ...
       $ docker logs -f pta_export_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it pta_export_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it pta_export_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/pta_export/
...
environment variables. See ``src/pta_export/conf/docker.py`` for
all settings.

.. code-block:: bash

    $ docker build -t pta_export
    $ docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    $ docker exec -it pta_export /app/src/manage.py createsuperuser
